...
import {
  ...
  ChevronDown, ChevronRight, ChevronLeft, Droplets, Settings, FileText, Upload
} from 'lucide-react';
import { BPLOT_PARAMETERS, CATEGORY_COLORS, CATEGORY_ORDER, CATEGORY_LABELS, VALUE_MAPPINGS, getDisplayValue, TIME_IN_STATE_CHANNELS, getDecimalPlaces, getSyncStateDisplay } from '../lib/bplotThresholds';
import parameterDefinitions4g from '../lib/parameterDefinitions4g.json';
import { formatDuration, calculateTimeInState } from '../lib/bplotProcessData';
import { extractEngineHourWindow, formatEngineHourValue } from '../lib/bplotEngineHours';
import { MAX_CHART_CHANNELS } from '../lib/chartResample';
import AppHeader from './AppHeader';
import { useThresholds } from '../contexts/ThresholdContext';
import BpltChartWorkspace from './charts/BpltChartWorkspace';
    ...
    <div className={hideHeader ? '' : 'min-h-screen bg-[#020617]'} style={{ color: 'white' }} ref={reportRef}>
      ...
      >
        ...
        {activeTab === 'charts' && (
          <>
          <BpltChartWorkspace
            normalizedData={effectiveProcessedData?.normalizedData || chartData || []}
            primaryNormalized={primaryBplotFile?.processed?.normalizedData || primaryBplotFile?.processed?.chartData || []}
            secondaryNormalized={secondaryBplotFile?.processed?.normalizedData || secondaryBplotFile?.processed?.chartData || []}
            selectedChannels={selectedChannels}
            onSelectedChannelsChange={setSelectedChannels}
            channelsByCategory={channelsByCategory}
            overlayEnabled={shouldOverlayCorrelatedPlots}
            automaticAlignmentOffset={automaticAlignmentOffset}
            highlightedChannel={highlightedChannel}
            selectedAlert={selectedAlert}
            activeCorrelatedRole={activeCorrelatedRole}
            fileBoundaries={fileBoundaries}
            shouldShowFileBoundaries={shouldShowFileBoundaries && showFileBoundaries}
          />
          {alerts.length > 0 && (
